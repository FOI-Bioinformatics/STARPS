...
int *reduce_loci_2(int *x,int nGenotype, int nloci, int nloci2, double *posi, int *pos);
...
void show_upd(double *fitness, int *idG, int nGenotype2);
...
void sparse_gen_pos(int *x,int *id,double *freqG,int *idG,int nG, int nGenotype,int nloci,int *postmp,int *pos);
...
SEXP WF2(SEXP nGenp, SEXP popsizep, SEXP expfactorp, SEXP mutratep, SEXP nlocip, SEXP verbosityp, SEXP nGenotypep, SEXP flag, SEXP inXp, SEXP posp, SEXP thresp, SEXP thresfrp, SEXP fitneffp){
 ...
 if(nmut > 0) posm2 = (int *) R_alloc(nmut,sizeof(int));
  ...
  printf("New number of genotypes after removing non-existant variants: %d\n",nGenotype2);
  /* allocating memory */
  if(!(xnew = (int *) calloc(nGenotype2*nloci,sizeof(int))) ||
     !(idG = (int *) calloc(nGenotype2,sizeof(int))) ||// ok
     !(fitness = (double *) calloc(nGenotype2,sizeof(double)))){ // ok
    printf("Error in allocating memory at line 201\n\n"); 
    return NULL;
  }
...
// ok
  ...
  if(!(postmp = (int *) calloc(nloci2,sizeof(int))) ||// ok
     !(id = (int *) calloc(nG,sizeof(int)))){
    printf("Error in allocating memory at line 218\n\n"); 
    return NULL;

  }
  x = reduce_loci_2(xnew,nGenotype,nloci,nloci2,posi,postmp);
  /* printf("nloci2: %d\n",nloci2); */
  printf("Genotypes:\n");
  ...
  sparse_gen_pos(x,id,freqG,idG,nG, nGenotype,nloci,postmp,pos);
  free(postmp);
  /* free(posi); */
//sparse_gen_pos(int *x,int *pos,int *id,double *freqG,int *idG,int nG, int nGenotype,int nloci,int *posi);
  ...
  /* Rprintf("fitness:\n"); */
  /* show(fitness,1,nGenotype2); */
  Rprintf("nr\tID\tfitness\n");
  show_upd(fitness,idG,nGenotype2);
  free(x);
  ...
  for(i = 1; i <= nGen; i++){
    ...
    if(verbosity==1) Rprintf("Number of genotypes in population after selection (nGenotype2): %d\nNumber of new mutations in population (nG2): %d\n",nGenotype2,nG2);
    /*   printf("id:\n"); */
    /* showint(id,1,nG); */
    /* allocate memory */
    /* tmp1 = nGenotype2; */
    ...
    if(verbosity==1) Rprintf("newgen = %d, max(idG) = %d\n",newgen,maxpos(idG,nGenotype));
	...
	return NULL;
      ...
      if(verbosity==1) Rprintf("nrmut[%d] = %d\n",i1,nrmut[i1]);
  ...
  Rprintf("Check freq after cultivation....\n");
  /* Rprintf("idGtmp:\n"); */
  /* showint(idGtmp,1,nGenotype2); */
  /* Rprintf("\nfitntmp:\n"); */
  /* show(fitntmp,1,nGenotype2); */
  Rprintf("nr\tID\tfitness\n");
  show_upd(fitntmp,idG,nGenotype2);
  /* Rprintf("idtmp:\n"); */
  /* showint(idtmp,1,nG2); */
  /* Rprintf("postmp:\n"); */
  ...
  l1 = nGenotype2 * (up + 1); // calculate size of vector
  /* Rprintf("l1 = %d\nhit2\n",l1); */
...
void sparse_gen_pos(int *x,int *id,double *freqG,int *idG,int nG, int nGenotype,int nloci,int *postmp,int *pos){ //,const gsl_rng * r

  int i,j,k = 0;
  //unsigned long int n;
  //n = (unsigned long int) size;
  /* Rprintf("Entering sparse_gen_pos function....\n"); */
  for(i = 0; i < nGenotype; i++)
    for(j = 0; j < nloci; j++){
      /* if(i == 0) Rprintf("loci %d, pos %f\n",j,posi[j]); */
  ...
  /* Rprintf("Leaving sparse_gen_pos function....\n"); */
}
void freq_sp(int *pos, int n1,int popsize,int nloci){
...
double *upd_geno(double *inX,int *x,double *freqtmp,double *fitness,int *idG,int nGenotype,int nloci,int nGenotype2){
  ...
  /* Rprintf("x in function upd_geno...\n"); */
  for(i = 0; i < nGenotype; i++){
    if(freqtmp[i] > 0.){
      /* Rprintf("%f ",inX[i]); */
      idG[k] = (int) inX[i];
	...
	/* Rprintf("%d ",x[j*nGenotype2+k]); */
      }
      /* Rprintf("%f %f",inX[(j+2)*nGenotype+i],inX[(j+3)*nGenotype+i]); */
      /* Rprintf("\n"); */
...
int reduce_loci_1(int *x,int nGenotype,int nloci){
  ...
  for(i = 0; i < nloci; i++){
    tmp = 0;
    for(j = 0; j < nGenotype; j++){
      //    Rprintf("%d ",x[i*nGenotype+j]);
      if(x[i*nGenotype+j]==1) tmp = 1;
    }
    //Rprintf("\n");
    if(tmp==1) nloci2++;
